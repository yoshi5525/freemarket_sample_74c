$(function(){
  ...
  if($('#category-preview').length != 0){
    $(".select-parent").on("change", function(){
      ...
      if($(this).val() == ""){
        ...
        $.ajax({
          ...
          $("#category-preview").append(`<select class="new-wrapper__main__input-select select-child" name="item[category_id]" id="item_category_id"><option value="">選択してください</option></select>`);
    ...
    $("#category-preview").on("change", ".select-child", function(){
      $(".select-grandchild").remove();
      if($(this).val() == ""){
        ...
        $.ajax({
          ...
          $("#category-preview").append(`<select class="new-wrapper__main__input-select select-grandchild" name="item[category_id]" id="item_category_id"><option value="">選択してください</option></select>`);
  ...
  else if(request.indexOf("new") != -1 || request.indexOf("new") != 0 || request.indexOf("edit") != -1 || request.indexOf("edit") != 0){
    $.ajax({
